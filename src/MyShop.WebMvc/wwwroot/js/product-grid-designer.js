(function () {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const lblPageIndex = document.getElementById("lblPageIndex");
        ...
        runSearchBtn.addEventListener("click", function () { runSearch(); });
        ...
        if (btnGo && goToPageInput) {
            btnGo.addEventListener("click", function () {
                let target = parseInt(goToPageInput.value, 10);
                if (isNaN(target) || target < 1) {
                    target = 1;
                }
                if (totalPages > 0 && target > totalPages) {
                    target = totalPages;
                }
                runSearch(target);
            });
        ...
        async function renderValueInput(conditionDiv, columnMeta) {
            ...
            if (columnMeta.inputType === "Dropdown" && columnMeta.lookupUrl) {
                try {
                    const response = await fetch(columnMeta.lookupUrl);
                    const apiResponse = await response.json();

                    if (apiResponse.success && Array.isArray(apiResponse.data)) {
                        const select = document.createElement("select");
                        select.className = "value-input value-dropdown";
                        ...
                        apiResponse.data.forEach(item => {
                            const opt = document.createElement("option");
                            opt.value = item[columnMeta.lookupValueField || "id"];
                            opt.textContent = item[columnMeta.lookupTextField || "name"];
                        ...
                        conditionDiv.dataset.columnType = String(columnMeta.columnType);
                        return;
                    }
                } catch (err) {
        ...
        function updatePager(page) {
            ...
            currentPageIndex = page.pageIndex || 1;
            totalPages = page.totalPages || 0;
            totalCount = page.totalCount || 0;

            const pageIndexInput = document.getElementById("pageIndex");
            if (pageIndexInput) {
                pageIndexInput.value = currentPageIndex;
            }

            if (goToPageInput) {
                goToPageInput.value = currentPageIndex;
                goToPageInput.min = "1";
                if (totalPages > 0) {
                    goToPageInput.max = String(totalPages);
                } else {
                    goToPageInput.removeAttribute("max");
                }
            }

            if (lblPageIndex) lblPageIndex.textContent = String(currentPageIndex);
            if (lblTotalPages) lblTotalPages.textContent = String(totalPages);
            if (lblTotalCount) lblTotalCount.textContent = String(totalCount);

            const disablePrev = currentPageIndex <= 1;
            const disableNext = totalPages === 0 || currentPageIndex >= totalPages;

            if (btnFirst) btnFirst.disabled = disablePrev;
            if (btnPrev) btnPrev.disabled = disablePrev;
            if (btnNext) btnNext.disabled = disableNext;
            if (btnLast) btnLast.disabled = disableNext;
        }

        // ------------- SEARCH -------------

        async function runSearch(forcedPageIndex) {
            const groupOperatorSelect = document.getElementById("groupOperator");
            const pageIndexInput = document.getElementById("pageIndex");
            const pageSizeInput = document.getElementById("pageSize");

            let targetPage = forcedPageIndex;
            if (targetPage == null) {
                targetPage = parseInt(pageIndexInput.value, 10) || 1;
            }
            ...
            // Input'u da sync et
            pageIndexInput.value = targetPage;
            ...
            groupDivs.forEach(function (groupDiv) {
                ...
                conditionDivs.forEach(function (condDiv) {
                    ...
                    const dropdownSelect = condDiv.querySelector(".value-dropdown");
                    if (dropdownSelect) {
                        value = dropdownSelect.value;
                    } else if (columnType === GridColumnType.Number) {
        ...
        function renderResults(apiResponse) {
            ...
            if (!apiResponse || !apiResponse.success || !apiResponse.data) {
                updatePager({ pageIndex: 1, totalPages: 0, totalCount: 0 });
                return;
            }
